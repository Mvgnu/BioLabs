...
Each handler enforces owner/admin access checks and reuses the shared serialization helper so API consumers receive consistent payloads. Celery tasks exposed through `services.cloning_planner.enqueue_pipeline` reuse the deterministic utilities in `services.sequence_toolkit` to keep primer design, restriction mapping, assembly heuristics, and QC aggregation aligned across API and worker surfaces. Tests under `tests/test_cloning_planner.py` cover session creation, stage recomputation, and finalization timestamps to keep the orchestration contract stable.
